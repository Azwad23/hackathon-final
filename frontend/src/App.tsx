...
import { Activity, AlertCircle, CheckCircle, Download, ExternalLink } from 'lucide-react'
  ...
  const startDownload = async () => {
    ...
    try {
      ...
      setJobs(prev => prev.map(j => 
        j.id === job.id 
          ? { ...j, status: result.status === 'completed' ? 'completed' : 'failed' }
          : j
      ))
    ...
    <div style={{ padding: '20px', fontFamily: 'system-ui' }}>
        ...
        {jobs.map(job => (
          <div key={job.id} style={{ 
            ...
            <div style={{ fontSize: '12px', color: '#666' }}>
              {job.traceId && (
                <span>Trace: {job.traceId.slice(0, 8)}...</span>
              )}
            </div>
          </div>
        ))}
      ...
      <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #ddd', borderRadius: '8px' }}>
        ...
        <button 
          ...
          style={{ marginBottom: '10px', padding: '8px 16px', cursor: 'pointer', backgroundColor: '#ff6b6b', color: 'white', border: 'none', borderRadius: '4px' }}
        >
          Trigger Test Error
        </button>
